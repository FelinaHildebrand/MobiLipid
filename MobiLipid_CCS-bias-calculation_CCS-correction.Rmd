---
title: "MobiLipid - A tool for CCS quality control and internal CCS correction using a CCS library for U^13^C labeled lipids"
author: "Felina Hildebrand"
date: "13.02.2024"
output:
  html_document: 
    toc: true
...
First, it calculates the CCS bias between measured CCS values of U^13^C labeled lipids compared to ^DT^CCS~N2~ values of a CCS library.
Secondly, the ^DT^CCS~N2~ library is used to generate up to 100 linear correction functions using 3 - 6 U^13^C labeled lipids for each lipid class and adduct combination. The measured IM values is converted with each function to a internal lipid class and adduct specific corrected CCS value. The CCS correction is only applied when at least 3 lipids are detected for a lipid class and adduct combination to ensure proper linear fitting.
As output the user gets a HTML file and a PDF file containing all gernerated data. Tables have to be accessed via the HTML file (in the PDF file thw first 10 rows for all tables are displayed). Additionally, the following CSV files are generated: (1) table with CCS bias (%) before correction (starting with: "CCS_bias_no_correction"), (2) table with all generated CCS correction functions (starting with: "Correction_functions"), (3) table with the mean CCS bias for each lipid class - adduct - combination for each correction function (starting with: "CCS_bias_mean_by_function"), (4) table with the mean CCS bias for each lipid class - adduct - combination over all correction functions with the same number of lipids used to gernerate the function (starting with: "CCS_bias_mean_all_functions"), (5) table with corrected CCS values for each lipid and each correction function (starting with: "Corrected_CCS_values"), and (6) table with mean corrected CCS values for each lipid and all correction functions with the same number of lipids used to gernerate the function (starting with: "Corrected_CCS_values_mean").'
---
...
## Column headers needed: File, LipidClass, LipidSpecies, Adduct, Label, IMS_value, CCS (LipidClass and LipidSpecies nomenclauture has to be as in library)
...
folder_path_new <- paste0(folder_path, "/Output_", filename)
...
report <- data_import[U13C_CCS_library, on=.(LipidClass,LipidSpecies,Adduct)]
...
results <- list()
report_bias <- report_bias[,c("File", "LipidClass", "LipidSpecies", "Polarity", "Adduct", "Label", "Formula", "Mz", "IMS_value", "CCS", "U13C_DTCCS", "CCS_bias")]
fwrite(report_bias, file = paste0("Output_", filename, "/CCS_bias_no_correction_",filename,".csv"))

results[["File"]] <- report_bias[,unique(File)]

report_bias <- report_bias[, c("LipidClass","LipidSpecies","Adduct","IMS_value","CCS","U13C_DTCCS","CCS_bias")]
...
rm(list=ls()[! ls() %in% c("U13C_CCS_library","data_import","report","results","filename")])
...
results_correction <- lapply(1:nrow(class_adduct_combination), function(k) {
  ...
  if (lipids_count > 2) {
    ...
    result_list_test <- lapply(1:length(vector_x), function(j){
      x <- vector_x[j]
      ...
      correction_function_dt <- lapply(1:n, function(i) {
        correction_func <- lm(U13C_DTCCS ~ IMS_value, data = dt_class_heavy[LipidSpecies %in% resampled_results[[i]]])
      ...
      dt_class_correction <- lapply(1:n, function(i) {
        ...
        subset[, CCS_corrected := (slope * IMS_value) + intercept]
      })
      ...
      dt_class_correction_long <- melt(dt_class_correction, id.vars = c("File", "LipidClass", "LipidSpecies", "Formula", "Mz", "Polarity", "Adduct", "Label", "U13C_DTCCS", "CCS", "CCS_corrected", "function_nr", "slope", "intercept"), measure=patterns("^CCS_bias"), value.name = "CCS_bias")
      ...
      correction_results <- correction_results[, c("LipidClass", "LipidSpecies", "Adduct", "Label", "Number_lipids_correction_function", "function_nr", "CCS_corrected")]
    ...
    names(result_list_test) <- vector_x
    result_list <- c(result_list, result_list_test)
...
rm(list=ls()[! ls() %in% c("U13C_CCS_library", "data_import", "report", "results", "filename","class_vector", "adduct_vector", "class")])
...
fwrite(correction_function_dt_all, file = paste0("Output_", filename, "/Correction_functions_",filename,".csv"))
...
bias_all_functions_all <- bias_all_functions_all[,c("LipidClass","Adduct","Number_lipids_correction_function","mean_CCS_bias","sd_CCS_bias")]
colnames(bias_all_functions_all)[colnames(bias_all_functions_all) == "Number_lipids_correction_function"] <- "Lipids_for_correction"
bias_all_functions_all[, Lipids_for_correction := as.character(Lipids_for_correction)]
bias_all_functions_all[Lipids_for_correction == "0", Lipids_for_correction := as.character("No correction")]
bias_all_functions_all <- unique(bias_all_functions_all)
fwrite(bias_all_functions_all, file = paste0("Output_", filename, "/CCS_bias_mean_all_functions_",filename,".csv"))
...
bias_by_function_all <- unique(bias_by_function_all)
fwrite(bias_by_function_all, file = paste0("Output_", filename, "/CCS_bias_mean_by_function_",filename,".csv"))
...
fwrite(correction_results_all, file = paste0("Output_", filename, "/Corrected_CCS_values_",filename,".csv"))
...
fwrite(correction_results_mean_all, file = paste0("Output_", filename, "/Corrected_CCS_values_mean_",filename,".csv"))

saveRDS(results, file = paste0("Output_", filename, "/R_results_",filename,".RData"))
...
lapply(3:length(results), function(i) {
  ...
  if (results[[i]][["Correction"]] == "Correction done.") {
    ...
    cat("\n###","Violin plot - CCS bias: All correction functions", "  \n")
